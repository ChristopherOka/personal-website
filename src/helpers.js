...
function renderNewPage(e, currentHeaderEl, skipAnimation = false) {
  ...
  document.getElementById(`${currentHeaderId}-body`).classList.remove("hidden");
...
function returnHome() {
  ...
  const header_body_els = document.querySelectorAll(header_body_ids.join(", "));
  ...
  setTimeout(() => {
    document.getElementById(`${currentHeaderId}-body`).classList.add("hidden");
...
function openExpandedCardByUrl() {
  ...
  if (pathname.includes("/experience") || cardId) {
    ...
    if (pathname.includes("/experience/design")) {
      ...
      const card = document.getElementById(`${pathname.split("/")[3]}-card`);
      if (!card) return;
      openExpandedCard(card, false);
      return;
    }
